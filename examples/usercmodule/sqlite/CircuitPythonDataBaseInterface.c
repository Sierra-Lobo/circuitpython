...
mp_obj_t usqlite_insertConfig(size_t n_args, const mp_obj_t* args)
{
	...
	usqlite_printf("usqlite insert config\n");
...
mp_obj_t usqlite_fetchConfig(size_t n_args,const mp_obj_t* args)
{
	...
	usqlite_printf("data after fetchConfig: %s\n", (char*)data);
	usqlite_printf("len after fetchConfig: %d\n", len);
	
	return mp_obj_bytearray_move(len, data);//mp_obj_new_bytearray(data, len);
}
...
mp_obj_t usqlite_getTableSize(size_t n_args, const mp_obj_t* args) {
	...
	return mp_obj_new_int(getTableSize(self, tableName));
}
